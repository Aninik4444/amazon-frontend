...
function Home() {
  ...
  useEffect(() => {
    const getProducts = async () => {
      setLoading(true);
      const res = await apiGetCall(`/products?page=${page}&limit=${limit}&name=${search}&select=-description&category=${category}&numericFilters=price>${priceRange[0]},price<${priceRange[1]},rating>${ratingRange[0]},rating<${ratingRange[1]}&inStock=${inStock}`);
